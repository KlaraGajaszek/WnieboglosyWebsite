...
const FILES_WITH_DEV_DEPENDENCIES = ['**/*.stories.tsx'];
...
module.exports = {
  ...
  rules: {
    ...
    'simple-import-sort/imports': [
      2,
      {
        groups: [
          ['^\\u0000'], // Side effects.
          ['^(react|remix)', '^@?\\w'], // Packages from node_modules. React-related packages will be first.
          ['^[^.]'], // Absolute imports.
          ['^\\.'], // Relative imports.
        ],
      },
  ...
  overrides: [
    {
      // files, which should contain default exports
      files: ['root.tsx', 'entry.server.tsx', 'vitest.config.ts', '**/routes/**', '*.stories.tsx'],
      rules: {
    ...
    },
    {
      ...
      files: ['**/?(*.)+(spec|test).ts?(x)', 'vitest.config.ts', 'setup-test-env.ts'],
      extends: [
